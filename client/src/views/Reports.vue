...
export default {
  ...
  setup() {
    ...
    // Computed summary stats — derived from raw data, not imperatively recalculated
    ...
    // Scanned once as a computed; getBarHeight reads this instead of rescanning per bar
    ...
    // Convert YYYY-MM to locale-aware short month + year (e.g. "Jan 2024" / "2024年1月")
    const formatMonth = (monthStr) => {
      ...
      if (isNaN(year) || isNaN(month) || month < 0 || month > 11) return monthStr
    ...
    const getBarHeight = (revenue) => {
      // maxMonthlyRevenue computed once, not rescanned per bar
